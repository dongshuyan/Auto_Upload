...
def audience_upload(web,file1,record_path,qbinfo,basic):
    ...
    try:
        #web.driver.find_elements(By.NAME,'douban_id')[0].click()
        web.driver.find_elements(By.NAME,'douban_id')[0].send_keys(file1.doubanurl)
    ...
    if 'check' in basic:
        ...
        if num==1:
            a=input('是否认发布，如果确认请回车，不发布请手动结束程序或者关闭终端')
    else:
